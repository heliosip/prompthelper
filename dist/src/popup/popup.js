...
const Popup = () => {
    ...
    return (<div className="w-96 p-4">
      {!user && !showLogin && (<button onClick={() => setShowLogin(true)} className="w-full mb-4 px-4 py-2 bg-gray-800 text-white rounded">
          Login
        </button>)}

      {showLogin && (<div className="mb-4 p-4 border rounded">
          <input type="text" placeholder="Username" className="w-full p-2 mb-2 border rounded" value={loginForm.username} onChange={e => setLoginForm(Object.assign(Object.assign({}, loginForm), { username: e.target.value }))}/>
          <input type="password" placeholder="Password" className="w-full p-2 mb-2 border rounded" value={loginForm.password} onChange={e => setLoginForm(Object.assign(Object.assign({}, loginForm), { password: e.target.value }))}/>
          <div className="flex justify-end gap-2">
            <button onClick={() => setShowLogin(false)} className="px-2 py-1 border rounded">
              Cancel
            </button>
            <button onClick={handleLogin} className="px-2 py-1 bg-blue-500 text-white rounded">
              Login
            </button>
          </div>
        </div>)}
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <button onClick={() => setShowNewForm(true)} className="px-2 py-1 bg-blue-500 text-white rounded">
          New Template
        </button>
      </div>

      {showNewForm && (<div className="mb-4 p-4 border rounded">
          <input type="text" placeholder="Template Name" className="w-full p-2 mb-2 border rounded" value={newTemplate.name} onChange={e => setNewTemplate(Object.assign(Object.assign({}, newTemplate), { name: e.target.value }))}/>
          <textarea placeholder="Template Content" className="w-full p-2 mb-2 border rounded" rows={4} value={newTemplate.content} onChange={e => setNewTemplate(Object.assign(Object.assign({}, newTemplate), { content: e.target.value }))}/>
          <div className="flex justify-end gap-2">
            <button onClick={() => setShowNewForm(false)} className="px-2 py-1 border rounded">
              Cancel
            </button>
            <button onClick={saveTemplate} className="px-2 py-1 bg-blue-500 text-white rounded">
              Save
            </button>
          </div>
        </div>)}
      ...
      {categories.map(category => (<div key={category} className="mb-4">
          ...
          <div className="space-y-2">
            {[...templates, ...userTemplates]
                ...
                .map((template) => (<div key={template.id} className="p-2 border rounded hover:bg-gray-50">
                  <div className="font-medium">{template.name}</div>
                  {template.isUserTemplate && (<div className="text-xs text-gray-500">Custom template</div>)}
                  <button onClick={() => {
                    chrome.tabs.query({ active: true, currentWindow: true }, ([tab]) => {
                        if (tab.id) {
